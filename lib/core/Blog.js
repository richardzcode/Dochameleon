...
class Blog {
  ...
  renderPage(pageNumber, pageSize) {
    ...
    const previous = pageNumber > 1
      ? site.url('/blog/page' + (pageNumber - 1) + '/index.html')
      : null;
  ...
  render(metadata) {
    ...
    const blogPostComp = <BlogPostLayout site={site} metadata={metadata} recent={recent} />
  ...
  _parseFile(file) {
    ...
    metadata.path = path.basename(file)
      ...
      .replace('-', '/')
      .replace(/\.md$/, '.html');
    metadata.permalink = this.site.url('blog/' + metadata.path);
