...
class Docs {
  ...
  processMetadata(file) {
    ...
    metadata.permalink = this.site.url('docs/' + metadata.path);
  ...
  render(metadata) {
    ...
    content = markdown.generateTOC(content);
    content = markdown.replaceLinksWithFull(content, this.metadatas, site.config);

    return renderToStaticMarkup(
      <DocsLayout site={site} metadata={metadata}>
        {content}
      </DocsLayout>
    );
  }
