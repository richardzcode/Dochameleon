...
class Site {
  ...
  checkHeaderLinks() {
    this.headerLinks = this.config.headerLinks
      .forEach(link => {
        if (link.doc && !dfs.exists(this.docs.rootPath)) { throw new Error(noDocsMessage); }
        if (link.page && !dfs.exists(this.pages.rootPath)) { throw new Error(noPagesMessage); }
        if (link.blog && !dfs.exists(this.blog.rootPath)) { throw new Error(noBlogMessage); }
      });
  }
